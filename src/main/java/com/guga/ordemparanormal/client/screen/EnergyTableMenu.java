...
public class EnergyTableMenu extends AbstractContainerMenu {
    private final EnergyTableBlockEntity blockEntity;
    private final Level level;
    public EnergyTableMenu(int id, Inventory inv, FriendlyByteBuf buf) {
        this(id, inv, inv.player.level.getBlockEntity(buf.readBlockPos()));
    }
    public EnergyTableMenu(int id, Inventory inv, BlockEntity entity) {
        super(OPMenuTypes.ENERGY_TABLE_MENU.get(), id);
        checkContainerSize(inv, 4);
        blockEntity = ((EnergyTableBlockEntity) entity);
        ...
        this.blockEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(cap -> {
            ...
            this.addSlot(new CurseTableResultSlot(cap, 3, 119, 28, ParanormalElement.ENERGIA));
        });
    }
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), pPlayer, OPBlocks.MESA_ENERGIA.get());
    }
