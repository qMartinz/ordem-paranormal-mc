...
public class DeathTableMenu extends AbstractContainerMenu {
    private final DeathTableBlockEntity blockEntity;
    private final Level level;
    public DeathTableMenu(int id, Inventory inv, FriendlyByteBuf buf) {
        this(id, inv, inv.player.level.getBlockEntity(buf.readBlockPos()));
    }
    public DeathTableMenu(int id, Inventory inv, BlockEntity entity) {
        super(OPMenuTypes.DEATH_TABLE_MENU.get(), id);
        checkContainerSize(inv, 4);
        blockEntity = ((DeathTableBlockEntity) entity);
        ...
        this.blockEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(cap -> {
            ...
            this.addSlot(new CurseTableResultSlot(cap, 3, 119, 28, ParanormalElement.MORTE));
        });
    }
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), pPlayer, OPBlocks.MESA_MORTE.get());
    }
