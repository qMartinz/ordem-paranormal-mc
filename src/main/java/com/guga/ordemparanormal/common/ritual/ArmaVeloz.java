...
public class ArmaVeloz extends AbstractRitual {
    ...
    public void onUseSelf(Level world, LivingEntity caster) {
        ...
        int newTicks = CurseHelper.getCurseTag(caster.getOffhandItem(), OPCurses.VELOZ.getId()).getInt("ticks") + extraTicks;
        CurseHelper.setTicks(CurseHelper.getCurseTag(caster.getOffhandItem(), OPCurses.VELOZ.getId()), newTicks);
    }
}
